...
public class Board {
    ...
    public Board(int numberOfShape){
        ...
        // 4각형 -> PathNode 30자리 배열
        // 5각형 -> PathNode 40자리 배열
        // 6각형 -> PathNode 50자리 배열
        switch(numberOfShape){
            ...
            case 5: {
                pathNodes=new PathNode[40]; //중복 인덱스롤 포함하여 40자리 배열
                for(int i=0; i<40; i++){
                    pathNodes[i]=new PathNode(i, 5);
                }
            } break;
            case 6: {
                pathNodes=new PathNode[50]; //중복 인덱스를 포함하여 50자리 배열
                for(int i=0; i<50; i++){
