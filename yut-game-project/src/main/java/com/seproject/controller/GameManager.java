public class GameManager {
    public void PlayGame(){} //전체 게임 흐름을 담당하는 로직이다. main에서 호출될 예정이다. 승리자가 나올 때까지 아래의 함수들을 게임 흐름에 맞게 순차적으로 호출한다. 
    ...
    private GameSetupUI gameSetupUI; // 조작할 UI
    ...
    public void isCaptureOrGroup(int srcNodeId){
        ...
        //그리고 isCapturedOrGrouped 기능이 넘 커지는 것 같아서 이 함수는 겹치는지 비교만하고, 잡거나 그룹화 하는 코드는 분리해서 함수로 만드는 거 어때요? -> iscapturedOrGrouped는 그냥 위치에 있는 말 조회해서 겹치는게 있나 있다면 상대편거가 섞여있나 섞여있다고? 너 시작위치로! 아니라고? 그냥 grouped1로 바꾸고 리턴 이게 끝이라서 굳이 쪼갤 필요는…?
    };

    public void checkWinCondition(){
        //매 턴이 끝날 때마다 호출된다. 만일 한 팀의 모든 말이 필드에 존재하지 않을 경//우(모든 말의 위치가 -1), 게임은 끝나고, 그 팀(개인전이므로 해당 플레이어가 된다)이 승리한다. 어느 팀(개인전이므로 해당 플레이어가 된다)도 그 조건을 만족시키지 못할 경우 //게임은 계속된다.
    };


    public int[] rollDice(boolean oneByOne, int selectedYut){
